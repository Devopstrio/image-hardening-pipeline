...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Image Overview', path: '/', icon: LayoutDashboard },
    { name: 'Hardening Console', path: '/hardening', icon: ShieldCheck },
    { name: 'Vulnerability Scan', path: '/scanning', icon: ShieldAlert },
    { name: 'SBOM Repository', path: '/sbom', icon: Binary },
    { name: 'Image Signing', path: '/signing', icon: PenTool },
    { name: 'Policy Validation', path: '/policies', icon: Lock },
    { name: 'Build History', path: '/history', icon: Box },
    { name: 'Supply Chain Audit', path: '/audit', icon: FileSearch },
    { name: 'Registry Governance', path: '/registry', icon: FileCode },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">HardenedChain</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">DevSecOps Lead</p>
                <p className="text-xs text-slate-500 truncate">Supply Chain Sec</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search images, CVEs, SBOMs, policies..." 
                className="w-full bg-slate-800/50 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all"
              />
            </div>
          </div>
